<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="list.aspx.cs" Inherits="HNNDB09_DOME.WEB.PAGE.XK.list" %>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('#GridView1').datagrid({
                ...
                { field: 'HNN09_XK_NAME', title: '学科', width: 100 }
                ]],
                //数据加载完成回掉的函数
        ...
        var toolbar = [{
            ...
            handler: function () {
                //alert('add')
                window.parent.openWin('350', '250', 'PAGE/XK/add.aspx?r=' + Math.random(), '学科添加');
            }
        }, '-', {
            text: '修改',
            iconCls: 'icon-cut',
            handler: function () {
                ...
                if (row != null) {
                    if (row.length > 1) {
                        ...
                        window.parent.openWin('350', '250', 'PAGE/XK/add.aspx?id=' + row.HNN09_XK_ID, '学科修改');
                    }
            ...
            handler: function () {
                ...
                if (row1!=null) {
                    ...
                    var a = [];
                    $(row).each(function () {
                        a.push(this.HNN09_XK_ID)
                    })
                    if (row != null) {
                        window.parent.sc('PAGE/XK/del.aspx?id=' + a.join(','))
                    }
                }
                else {
                    //$.messager.alert("请选择一项");
    ...
    <form id="form1" runat="server" style="border :none">
        <div data-options="region:'north',border:false" style="height: 40px; background: #ffffff; padding: 10px; overflow-y: hidden">
           <%--<span style ="display:none"> <asp:Button ID="delet" runat="server" Text="Button" OnClick="delet_Click" /></span>--%>
            <div>
                &nbsp;
                请输入学科名：<input id="xk_no" class="easyui-textbox"  style="width: 100px;height:21px" type="text" />&nbsp;
